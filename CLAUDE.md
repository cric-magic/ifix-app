...
- **List/index views** (e.g. `ProductsPage`, `UnitsListPage`) — filters and the primary action button (`Create Product`, `Add Unit`) render in their own row **above** the panel, not inside it. The panel itself has no header row. The page root takes `className="ifix-fill-page"` and the table `scroll.y: '100%'`, so the panel fills the main area with the column headers and pagination pinned and only the rows scrolling (see `.ifix-fill-page` in `index.css`); a table shared with a detail view opts in through a flag instead (`BranchTable`'s `fillHeight`).
...
Filters follow the action button, not the page: on a detail view they belong **inside** the panel too, grouped with the action button on the right of the same header row — title on the left, controls on the right (see `ContractTemplatesTab`, which passes them to `ContractTemplateTable`'s `filters` prop). A filter only narrows its own panel's table, so floating it above would read as a control for the whole page — which is exactly what a list view's filter row *is* — and giving it a second bar inside the panel just doubles the chrome. The controls shrink to fit that row rather than the row growing: the search field flexes down and drops to a shorter placeholder. Once the panel itself is too narrow for one row (under ~720px, measured on the panel via a container query, not the viewport), the header stacks instead — title above, controls below, all start-aligned — via the `.ifix-stackable-panel` / `.ifix-stackable-header` classes in `index.css`.
